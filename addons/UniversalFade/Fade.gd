...
static func fade_out(time := 1.0, color := Color.BLACK, pattern := "", reverse := false, smooth := true) -> Fade:
...
static func fade_in(time := 1.0, color := Color.BLACK, pattern := "", reverse := true, smooth := true) -> Fade:
	var fader := _create_fader(color, pattern, reverse, smooth)
